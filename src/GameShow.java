...
public class GameShow {
    ...
    static String[][] songDataSet = {
            ...
            {"Everybody Want to Rule The world", "Pop", "24.wav"},
            {"Ho Hey", "Pop", "25.wav"},
    ...
    static void rock() throws IOException, InterruptedException, UnsupportedAudioFileException, LineUnavailableException {
        ...
        int[] songIndexes = new int[7];
        do{
            songIndexes = shuffle(0, 14, 7);
        }while(checkDuplicates(songIndexes));
        String a = "";
    ...
    static void popMusic() throws UnsupportedAudioFileException, LineUnavailableException, IOException, InterruptedException {
        ...
        for (int i = 0; i <    numberOfPopSongs; i++) {
            if (songDataSet[numberOfRockSongs + i][1].equals("Pop"))
                songs[i] = new Songs(songDataSet[numberOfRockSongs + i][0], songDataSet[numberOfRockSongs + i ][1], songDataSet[numberOfRockSongs + i][2]);
        }
    ...
    static void jpon() throws UnsupportedAudioFileException, LineUnavailableException, IOException, InterruptedException {
        ...
        for (int i = 0; i <  numberOfRockSongs; i++) {
            if (songDataSet[numberOfRockSongs + numberOfPopSongs + i][1].equals("JPop"))
                songs[i] = new Songs(songDataSet[numberOfRockSongs + numberOfPopSongs + i][0],
                        songDataSet[numberOfRockSongs + numberOfPopSongs + i][1],
                        songDataSet[numberOfRockSongs + numberOfPopSongs][2]);
        }
    ...
    static void menu() throws UnsupportedAudioFileException, LineUnavailableException, IOException, InterruptedException {
        ...
        while (true) {
            ...
            System.out.println("Press 1 for rules, 2 for play and 3 for exit");
            input = in.nextLine();
            if (input.equals("1")) {
